...
class EmotionInputController {
        ...
        try {
            ...
            try {
                ...
                if (typeof DataStorage !== 'undefined') {
                    ...
                    // Force local mode for permanent local saving
                    if (this.dataStorage.setStorageMode) {
                        await this.dataStorage.setStorageMode('local');
                    }
                } else {
    ...
    /**
     * Map face-api.js expressions to our emotion categories - DELEGATED TO CameraManager
     */
    mapExpressionToEmotion(expression) {
        // Delegate to CameraManager if available
        if (window.cameraManager && typeof window.cameraManager.mapExpressionToEmotion === 'function') {
            return window.cameraManager.mapExpressionToEmotion(expression);
        } else {
            // Fallback mapping
            const emotionMap = {
                'happy': 'happy',
    ...
    updateFaceDetectionUI(data) {
        ...
        if (window.uiManager) {
            if (!data) {
                window.uiManager.updateFaceDetected('No');
                window.uiManager.updateCameraEmotion('-');
                window.uiManager.updateCameraConfidence(0);
            } else {
                window.uiManager.updateFaceDetected('Yes');
                window.uiManager.updateCameraEmotion(data.emotion);
                window.uiManager.updateCameraConfidence(data.confidence);
            }
        } else {
            // Fallback for direct element updates
            if (!data) {
                if (this.elements.faceDetected) this.elements.faceDetected.textContent = 'No';
                if (this.elements.cameraEmotion) this.elements.cameraEmotion.textContent = '-';
                if (this.elements.cameraConfidenceMeter) this.elements.cameraConfidenceMeter.style.width = '0%';
    ...
    startVoiceAnalysis() {
        ...
        const analyzeVoice = async () => {
            ...
            try {
                ...
                // Gunakan EmotionAnalyzer untuk analisis emosi audio
                if (window.emotionAnalyzer && typeof window.emotionAnalyzer.analyzeAudio === 'function') {
                    const emotion = await window.emotionAnalyzer.analyzeAudio(dataArray);
                    ...
                    // Fallback jika analyzer tidak ada
                    this.updateVoiceEmotionUI({ emotion: 'neutral', confidence: 0.5 });
                }
                // Update spectrogram
        ...
        // Run analysis loop
        this.voiceAnalysisInterval = setInterval(analyzeVoice, this.config.audioAnalysisInterval);
    }

    updateVoiceEmotionUI(emotionData) {
        // Delegate to UIManager if available
        if (window.uiManager) {
            window.uiManager.updateAudioEmotion(emotionData.emotion);
            window.uiManager.updateAudioConfidence(emotionData.confidence);
            window.uiManager.updateVoiceQuality(emotionData.confidence > 0.7 ? 'Good' : 
    ...
    async analyzeTextSentiment(text) {
        // Delegate to TextAnalyzer if available
        if (window.textAnalyzer && typeof window.textAnalyzer.performAnalysis === 'function') {
            return window.textAnalyzer.performAnalysis(text);
        } else {
    ...
    updateTextAnalysisUI(emotionData, text) {
        // Delegate to UIManager if available
        if (window.uiManager && typeof window.uiManager.updateTextResults === 'function') {
            window.uiManager.updateTextResults({
    ...
    async saveEmotionData(data) {
        try {
            data = this.normalizeEmotionData(data);
            await this.dataStorage.saveEmotionData(data);
            this.showNotification('Data emosi berhasil disimpan!', 'success');
            this.addToHistory(data);
        } catch (error) {
            this.showError('Gagal auto-save data emosi: ' + error.message);
        }
    }
    ...
    /**
     * Current Emotion Display - DELEGATED TO UIManager
     */
    updateCurrentEmotion(emotion, source, confidence) {
        // Delegate to UIManager if available
        if (window.uiManager && typeof window.uiManager.updateEmotionDisplay === 'function') {
            window.uiManager.updateEmotionDisplay({
                emotion: emotion,
                source: source,
                confidence: confidence
            });
            ...
            // Enable submit button untuk semua emosi (termasuk 'neutral')
            if (this.elements.submitDataBtn) {
                const hasValidEmotion = !!emotion;
    ...
    /**
     * Utility Functions - DELEGATED TO UIManager
     */
    ...
    /**
     * Simple localStorage methods - DELEGATED TO DataManager
     */
    saveToLocalStorage(data) {
        // Delegate to DataManager if available
        if (window.dataManager && typeof window.dataManager.saveEmotionData === 'function') {
            return window.dataManager.saveEmotionData(data);
        } else {
    ...
    getFromLocalStorage() {
        // Delegate to DataManager if available
        if (window.dataManager && typeof window.dataManager.getEmotionData === 'function') {
            return window.dataManager.getEmotionData();
        } else {
            // Fallback localStorage
            try {
    ...
    async exportFromLocalStorage(format, filters = {}) {
        ...
        if (window.dataManager && typeof window.dataManager.exportData === 'function') {
            return window.dataManager.exportData(format, filters);
        } else {
            // Fallback export
            try {
    ...
    generateId() {
        // Delegate to DataManager if available
        if (window.dataManager && typeof window.dataManager.generateId === 'function') {
    ...
    /**
     * Submit current emotion data (manual or auto-save)
     */
    async submitCurrentData() {
        try {
            let data = this.collectCurrentEmotionData ? this.collectCurrentEmotionData() : {};
            data = this.normalizeEmotionData(data);
            await this.dataStorage.saveEmotionData(data);
            this.showNotification('Data emosi berhasil disimpan!', 'success');
            this.addToHistory(data);
        } catch (error) {
            this.showError('Gagal menyimpan data emosi: ' + error.message);
        }
    }
    ...
    /**
     * Normalize and validate emotion data before saving
     */
    normalizeEmotionData(raw) {
        // Default values and mapping
        ...
        let dominantEmotion = raw.dominantEmotion || raw.emotion || this.currentSession.currentEmotion || 'neutral';
        if (!validEmotions.includes(dominantEmotion)) dominantEmotion = 'neutral';
        let source = raw.source || raw.emotionSource || this.currentSession.emotionSource || 'camera';
        ...
        // Gabungkan semua field yang diperlukan
        return {
            ...raw,
            dominantEmotion,
            source,
