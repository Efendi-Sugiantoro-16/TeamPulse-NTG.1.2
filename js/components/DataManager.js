...
class DataManager {
    ...
    normalizeEmotionData(raw) {
        ...
        let dominantEmotion = raw.dominantEmotion || raw.emotion || raw.feeling || raw.mood || 'neutral';
        if (!validEmotions.includes(dominantEmotion)) dominantEmotion = 'neutral';
        let source = raw.source || raw.emotionSource || raw.inputType || raw.type || 'camera';
        if (!validSources.includes(source)) source = 'camera';
        let confidence = typeof raw.confidence === 'number' ? raw.confidence : (raw.confidence ? parseFloat(raw.confidence) : (raw.score ? parseFloat(raw.score) : 0.8));
        if (isNaN(confidence) || confidence < 0 || confidence > 1) confidence = 0.8;
        let timestamp = raw.timestamp || raw.createdAt || raw.date || new Date().toISOString();
        return {
            ...raw,
            dominantEmotion,
            source,
